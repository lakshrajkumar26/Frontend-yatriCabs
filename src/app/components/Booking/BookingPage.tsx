...
const BookingPage = () => {
  ...
  const [sourceAddress, setSourceAddress] = useState<string>(source.address || "");
  const [destinationAddress, setDestinationAddress] = useState<string>(destination.address || "");
  ...
  const onSourceAddressClicked = async (ele: any) => {
    ...
    dispatch(
      setSourceCoordinates({
        ...
        address: ele.name, // ✅ save address too
      })
    );
  };
            ...
            <div className={styles.suggestions}>
              <ul>
                {destinationAddressList.map((ele: any, index: number) => (
                  <li key={index} onClick={() => onDestinationAddressClicked(ele)}>
                    <span className={styles.eachSuggestion}>
                      {ele.name} - <span>{ele.place_formatted}</span>
                    </span>
                  </li>
